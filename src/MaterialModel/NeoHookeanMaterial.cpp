...
double NeoHookeanMaterial<SFF>::stretchingEnergy(const MeshConnectivity& mesh,
    ...
    if (hessian) {
        ...
        *hessian += term1 / deta * aderivadj.transpose() * aderiv;
        ...
        for (int i = 0; i < 4; ++i) *hessian += (term1 * ainv(i) + lameBeta * abarinv(i)) * ahess[i];
...
//}
 ...
 double NeoHookeanMaterial<SFF>::bendingEnergy(
     ...
     Matrix2d a = firstFundamentalForm(mesh, curPos, face, (derivative || hessian) ? &aderivsmall : NULL,
                                       hessian ? &ahesssmall : NULL);
     ...
     if (result < 0) {
        std::cout << "bending energy is negative: " << result << std::endl;
        std::cout << "M: \n" << M << std::endl;
        std::cout << "a^{-1}: \n" << aadj / deta << std::endl;
        std::cout << "b: \n" << b << std::endl;
        std::cout << "bbar: \n" << rs.bbars[face] << std::endl;
        std::cout << "M.trace(): " << M.trace() << std::endl;
        std::cout << "M*M trace: " << (M * M).trace() << std::endl;
        std::cout << "a: \n" << a << std::endl;
        std::cout << "b: \n" << b << std::endl;
        std::cout << "deta: " << deta << std::endl;
        std::cout << "detabar: " << detabar << std::endl;
        std::cout << "lameAlpha: " << lameAlpha << std::endl;
        std::cout << "lameBeta: " << lameBeta << std::endl;
        exit(1);
     }
     ...
     if (derivative) {
         ...
         *derivative += term3 * m3(1, 1) * bderiv.row(3);
         ...
         Matrix2d m4 = badj * rs.abars[face] * bbaradj / detabar;
         ...
         *derivative += term5 * aadj(1, 1) * bderiv.row(3);
         ...
         double term6 = lameAlpha * (abaradj * rs.bbars[face] / detabar - aadj * b / deta).trace() * 1.0 / pow(deta, 2) *
                        (aadj * b).trace();
     ...
     if (hessian) {
         ...
         *hessian += term1 * m1(1, 1) * bhess[3];
         Matrix2d m2 = aadj * b;
         *hessian +=
             term1 * (m2(0, 0) * aderiv.row(3).transpose() + m2(0, 1) * -aderiv.row(2).transpose()) * bderiv.row(0);
         *hessian +=
             term1 * (m2(0, 0) * -aderiv.row(1).transpose() + m2(0, 1) * aderiv.row(0).transpose()) * bderiv.row(1);
         *hessian +=
             term1 * (m2(1, 0) * aderiv.row(3).transpose() + m2(1, 1) * -aderiv.row(2).transpose()) * bderiv.row(2);
         *hessian +=
             term1 * (m2(1, 0) * -aderiv.row(1).transpose() + m2(1, 1) * aderiv.row(0).transpose()) * bderiv.row(3);
         ...
         *hessian += term1 * (aadj(1, 0) * bderiv.row(1).transpose() + aadj(1, 1) * bderiv.row(3).transpose()) *
         ...
         Matrix2d m3 = a * badj;
         *hessian +=
             term1 * (m3(1, 0) * aderiv.row(1).transpose() + m3(1, 1) * aderiv.row(3).transpose()) * bderiv.row(0);
         *hessian +=
             term1 * -(m3(0, 0) * aderiv.row(1).transpose() + m3(0, 1) * aderiv.row(3).transpose()) * bderiv.row(1);
         *hessian +=
             term1 * -(m3(1, 0) * aderiv.row(0).transpose() + m3(1, 1) * aderiv.row(2).transpose()) * bderiv.row(2);
         *hessian +=
             term1 * (m3(0, 0) * aderiv.row(0).transpose() + m3(0, 1) * aderiv.row(2).transpose()) * bderiv.row(3);
         ...
         *hessian += term3 * m5(1, 1) * ahess[3];
         ...
         Matrix2d m6 = badj * a;
         *hessian +=
             term3 * (m6(0, 0) * bderiv.row(3).transpose() + m6(0, 1) * -bderiv.row(2).transpose()) * aderiv.row(0);
         *hessian +=
             term3 * (m6(0, 0) * -bderiv.row(1).transpose() + m6(0, 1) * bderiv.row(0).transpose()) * aderiv.row(1);
         *hessian +=
             term3 * (m6(1, 0) * bderiv.row(3).transpose() + m6(1, 1) * -bderiv.row(2).transpose()) * aderiv.row(2);
         *hessian +=
             term3 * (m6(1, 0) * -bderiv.row(1).transpose() + m6(1, 1) * bderiv.row(0).transpose()) * aderiv.row(3);
         ...
         *hessian += term3 * (badj(1, 0) * aderiv.row(1).transpose() + badj(1, 1) * aderiv.row(3).transpose()) *
         ...
         Matrix2d m7 = b * aadj;
         *hessian +=
             term3 * (m7(1, 0) * bderiv.row(1).transpose() + m7(1, 1) * bderiv.row(3).transpose()) * aderiv.row(0);
         *hessian +=
             term3 * -(m7(0, 0) * bderiv.row(1).transpose() + m7(0, 1) * bderiv.row(3).transpose()) * aderiv.row(1);
         *hessian +=
             term3 * -(m7(1, 0) * bderiv.row(0).transpose() + m7(1, 1) * bderiv.row(2).transpose()) * aderiv.row(2);
         *hessian +=
             term3 * (m7(0, 0) * bderiv.row(0).transpose() + m7(0, 1) * bderiv.row(2).transpose()) * aderiv.row(3);
         ...
         *hessian +=
             term8 * (m11(1, 0) * aderiv.row(1).transpose() + m11(1, 1) * aderiv.row(3).transpose()) * bderiv.row(0);
         *hessian +=
         ...
         *hessian +=
             term8 * (m11(0, 0) * aderiv.row(0).transpose() + m11(0, 1) * aderiv.row(2).transpose()) * bderiv.row(3);
         ...
         *hessian += term15 * aadjda.transpose() * aadjdb;
         ...
         double term16 = lameAlpha * 1.0 * (abaradj * rs.bbars[face] / detabar - aadj * b / deta).trace() /
                         pow(deta, 2) * (aadj * b).trace();
         ...
         *hessian += term17 * badjda.transpose() * aadjda;
         ...
         double term18 = lameAlpha * -2.0 * (abaradj * rs.bbars[face] / detabar - aadj * b / deta).trace() /
                         pow(deta, 3) * (aadj * b).trace();
...
template class NeoHookeanMaterial<MidedgeAngleGeneralFormulation>;
template class NeoHookeanMaterial<MidedgeAngleGeneralSinFormulation>;
template class NeoHookeanMaterial<MidedgeAngleGeneralTanFormulation>;
};  // namespace LibShell