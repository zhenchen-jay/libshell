...
std::vector<std::pair<double, std::string>> parseTestCases(const std::string& parent_dir, const int num_verts) {
    // the thickness is in the form 0.000100, 0.001000, 0.010000, 0.100000, 1.000000
    // we want to parse all the sub directories under the parent_dir
    std::vector<std::string> test_cases;
    std::vector<double> thicknesses;
    for (const auto& entry : std::filesystem::directory_iterator(parent_dir)) {
        if (entry.is_directory()) {
            if (entry.path().filename().string().find("V_" + std::to_string(num_verts)) != std::string::npos) {
                test_cases.push_back(entry.path().string());
                thicknesses.push_back(std::stod(entry.path().filename().string().substr(entry.path().filename().string().find("h_") + 2, 7)));
            }
        }
    }
    // sort the test cases by the thicknesses
    std::vector<std::pair<double, std::string>> sorted_test_cases;
    for (size_t i = 0; i < test_cases.size(); i++) {
        sorted_test_cases.push_back({thicknesses[i], test_cases[i]});
    }
    std::sort(sorted_test_cases.begin(), sorted_test_cases.end());
...
int main(int argc, char* argv[]) {
    ...
    app.add_option("-n, --num_verts", args.num_verts, "Number of vertices");
    app.add_flag("-g, --with_gui", args.with_gui, "Whether to show the gui");
    CLI11_PARSE(app, argc, argv);

    std::vector<std::pair<double, std::string>> test_cases = parseTestCases(args.results_dir, args.num_verts);
    for (const auto& [thickness, test_case] : test_cases) {
        std::cout << "thickness: " << thickness << ", test_case: " << test_case << std::endl;
    }    
    ...
    std::vector<std::vector<Eigen::MatrixXd>> flatV_list;
    std::vector<std::vector<Eigen::MatrixXd>> V3d_list;
    std::vector<std::vector<Eigen::MatrixXi>> F_list;
    std::vector<std::vector<Eigen::VectorXd>> edge_dofs_list;
    std::vector<std::vector<Config>> configs_list;
    std::vector<std::vector<OptimizationLog>> opt_logs_list;
    std::vector<std::vector<std::vector<Eigen::Vector3d>>> face_edge_vecs_list;
    std::vector<Eigen::VectorXd> surface_dist_list;
    std::vector<Eigen::VectorXd> edge_vec_dist_list;
    std::vector<CompResult> comp_results_list;
    ...
    for (const auto& [thickness, test_case] : test_cases) {
        ...
        auto parse_files = [&]() {
            for (int i = 0; i < 2; i++) {
                ...
                std::string config_file_path = test_case + "/" + model_name + "/initial_config.txt";
                std::string optimization_log_file_path = test_case + "/" + model_name + "/optimization_log.txt";
                std::string rest_mesh_file_path = test_case + "/" + model_name + "/rest_mesh.obj";
                std::string converged_mesh_file_path = test_case + "/" + model_name + "/converged_mesh.obj";
                std::string edge_dofs_file_path = test_case + "/" + model_name + "/edge_dofs.txt";
        ...
        if (!parse_files()) {
            std::cerr << "Failed to parse files: " << test_case << std::endl;
            break;
        }

        // append the current mesh to the list
        flatV_list.push_back(flatV);
        V3d_list.push_back(V3d);
        F_list.push_back(F);
        edge_dofs_list.push_back(edge_dofs);
        configs_list.push_back(configs);
        opt_logs_list.push_back(opt_logs);
        face_edge_vecs_list.push_back(face_edge_vecs);
        ...
        comp_result.edge_vec_dist =
            computeEdgeVecDist(flatV[0], F[0], face_edge_vecs[0], face_edge_vecs[1], edge_vec_dist);

        surface_dist_list.push_back(surface_dist);
        edge_vec_dist_list.push_back(edge_vec_dist);
        comp_results_list.push_back(comp_result);

        // save this to a file
        // std::ofstream out_file(test_case + "/comp_result.txt");
    ...
    if (args.with_gui) {
        ...
        polyscope::init();
        polyscope::SurfaceMesh* rest_surface_mesh = nullptr;
        bool visualize_rest_mesh = false;
        bool visualize_s1_sin_mesh = true;
        bool visualize_s2_sin_mesh = true;

        bool visualize_rest_mesh_vec = false;
        bool visualize_s1_sin_vec = false;
        bool visualize_s2_sin_vec = false;

        auto frame_rendering = [&](int current_frame) {
            // visualize the difference on the rest mesh
            rest_surface_mesh = polyscope::registerSurfaceMesh("rest mesh", flatV_list[current_frame][0], F_list[current_frame][0]);
            rest_surface_mesh->setEnabled(visualize_rest_mesh);

            auto surface_dist_quantity = rest_surface_mesh->addVertexScalarQuantity("surface_dist", surface_dist_list[current_frame]);
            surface_dist_quantity->setMapRange({0, 1e-6});

            // we should average the edge_vec_dist to the face distance for visualization
            Eigen::VectorXd face_edge_vec_dist(F_list[current_frame][0].rows());
            face_edge_vec_dist.setZero();
            for (int i = 0; i < F_list[current_frame][0].rows(); i++) {
                for (int j = 0; j < 3; j++) {
                    face_edge_vec_dist(i) += edge_vec_dist_list[current_frame](3 * i + j);
                }
            }
            auto face_edge_vec_dist_quantity = rest_surface_mesh->addFaceScalarQuantity("face_edge_vec_dist", face_edge_vec_dist);
            face_edge_vec_dist_quantity->setMapRange({0, 1e-5});

            LibShell::MeshConnectivity cur_mesh(F_list[current_frame][0]);

            std::vector<Eigen::Vector3d> face_edge_midpts(3 * F_list[current_frame][0].rows());
            for (int i = 0; i < F_list[current_frame][0].rows(); i++) {
                for (int j = 0; j < 3; j++) {
                    int eid = cur_mesh.faceEdge(i, j);
                    face_edge_midpts[3 * i + j] =
                        (flatV_list[current_frame][0].row(cur_mesh.edgeVertex(eid, 0)) + flatV_list[current_frame][0].row(cur_mesh.edgeVertex(eid, 1))) / 2.0;
                }
            }
            auto pt_mesh = polyscope::registerPointCloud("face edge midpts", face_edge_midpts);
            pt_mesh->setEnabled(visualize_rest_mesh_vec);
            pt_mesh->setPointRadius(0);

            // draw the S1 and S2 sin meshes
            for (int i = 0; i < 2; i++) {
                auto model_name = model_names[i];
                auto surface_mesh = polyscope::registerSurfaceMesh(model_name, V3d_list[current_frame][i], F_list[current_frame][i]);
                surface_mesh->setEnabled(i == 0 ? visualize_s1_sin_mesh : visualize_s2_sin_mesh);

                std::vector<Eigen::Vector3d> face_edge_midpts(3 * F_list[current_frame][i].rows());
                for (int j = 0; j < F_list[current_frame][i].rows(); j++) {
                    for (int k = 0; k < 3; k++) {
                        int eid = cur_mesh.faceEdge(j, k);
                        face_edge_midpts[3 * j + k] =
                            (V3d_list[current_frame][i].row(cur_mesh.edgeVertex(eid, 0)) + V3d_list[current_frame][i].row(cur_mesh.edgeVertex(eid, 1))) / 2.0;
                    }
                }
                auto pt_mesh_3d = polyscope::registerPointCloud(model_name + " face edge midpts", face_edge_midpts);
                pt_mesh_3d->setEnabled(i == 0 ? visualize_s1_sin_vec : visualize_s2_sin_vec);
                pt_mesh_3d->setPointRadius(0);

                auto vec_3d = pt_mesh_3d->addVectorQuantity(model_name + " vec", face_edge_vecs_list[current_frame][i]);
                vec_3d->setEnabled(i == 0 ? visualize_s1_sin_vec : visualize_s2_sin_vec);
            }
        };

        int current_frame = 0;
        frame_rendering(current_frame);

        polyscope::state::userCallback = [&]() {
            ImGui::Text("Current thickness: %f", test_cases[current_frame].first);

            if(ImGui::Checkbox("Visualize Rest Mesh", &visualize_rest_mesh)) {
                frame_rendering(current_frame);
            }
            if(ImGui::Checkbox("Visualize S1 sin", &visualize_s1_sin_mesh)) {
                frame_rendering(current_frame);
            }
            if(ImGui::Checkbox("Visualize S2 sin", &visualize_s2_sin_mesh)) {
                frame_rendering(current_frame);
            }

            if(ImGui::Checkbox("Visualize Rest Mesh Vec", &visualize_rest_mesh_vec)) {
                frame_rendering(current_frame);
            }
            if(ImGui::Checkbox("Visualize S1 sin Vec", &visualize_s1_sin_vec)) {
                frame_rendering(current_frame);
            }
            if(ImGui::Checkbox("Visualize S2 sin Vec", &visualize_s2_sin_vec)) {
                frame_rendering(current_frame);
            }
            ...
            if (ImGui::SliderInt("Current Frame", &current_frame, 0, flatV_list.size() - 1)) {
                frame_rendering(current_frame);
            }
        };
