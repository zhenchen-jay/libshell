...
Eigen::Vector3d external_force = Eigen::Vector3d(0, 0, -10);
...
double external_force_energy(const std::vector<Eigen::Matrix2d>& abars, const Eigen::MatrixXd& cur_pos, const LibShell::MeshConnectivity& cur_mesh, const Eigen::Vector3d& force, Eigen::VectorXd* grad = nullptr) {
    double total_energy = 0;
    std::vector<double> vert_areas(cur_pos.rows(), 0);
    for (int i = 0; i < cur_mesh.nFaces(); i++) {
        double area = std::sqrt(abars[i].determinant()) / 2;
        for (int j = 0; j < 3; j++) {
            vert_areas[cur_mesh.faceVertex(i, j)] += area / 3.0 * shell_thickness;
        }
    }
    if (grad) {
        grad->resize(cur_pos.rows() * 3);
        grad->setZero();
    }

    for (int i = 0; i < cur_pos.rows(); i++) {
        total_energy += -vert_areas[i] * force.dot(cur_pos.row(i).segment<3>(0).transpose());
        if (grad) {
            grad->segment<3>(i * 3) -= force * vert_areas[i];
        }
    }
...
void optimizeFullDOFs(ShellEnergy& energy,
    ...
    auto obj_func = [&](const Eigen::VectorXd& var, Eigen::VectorXd* grad, Eigen::SparseMatrix<double>* hessian,
        ...
        Eigen::VectorXd external_force_grad;
        double external_force_energy_val = external_force_energy(abars, pos, cur_mesh, external_force, &external_force_grad);
        total_energy += external_force_energy_val;

        if (grad) {
            grad->segment(0, nposdofs) += external_force_grad;
            *grad = P * (*grad);
        }
...
int main(int argc, char* argv[]) {
    ...
    app.add_option("-f, --force_magnitude", args.force_magnitude, "Magnitude of the external force");
    ...
    auto initialize_all = [&]() {
        ...
        external_force = Eigen::Vector3d(0, 0, force_magnitude);
        std::tie(dir_energy_model, extra_energy_terms) = initialization(
            rest_mesh, flatV, cur_mesh, shell_thickness, young, poisson, edge_dofs, rest_state, sff_type, material_type);
        ...
        for (int i = 0; i < flatV.rows(); i++) {
            ...
            if (r <= inner_radius * (1 + 1e-6)) {
                inner_verts.push_back(i);
            } else if (r >= middle_radius * (1 - 1e-3)) {
    ...
    auto initialize_rendering = [&]() {
        ...
        std::vector<Eigen::Vector3d> outer_verts_pos;
        for (int i = 0; i < outer_verts.size(); i++) {
            outer_verts_pos.push_back(V3d.row(outer_verts[i]));
        }
        auto fixed_pt_mesh = polyscope::registerPointCloud(prefix_name + "Fixed verts", outer_verts_pos);
    ...
    polyscope::state::userCallback = [&]() {
        ...
        if (ImGui::InputDouble("External Force Magnitude", &force_magnitude)) {
